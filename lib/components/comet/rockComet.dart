...
class RockComet extends Comet {
  ...
  RockComet(SpaceSurvivalGame game, String cometId, Level level,
      CometLevel cometLevel, double weightComponent, double heightComponent)
      : super(game, cometId, level, cometLevel, weightComponent,
            heightComponent) {
    cometRect = Rect.fromLTWH(cometLevel.setXRandomPosition,
        cometLevel.setYRandomPosition, weightComponent, heightComponent);

    //cometSprite = List<Sprite>(); (Deprecated)
    cometSprite = [];
    ...
    //cometLockSprite = List<Sprite>(); (Deprecated)
  ...
  static void setComponentBehaviours() {
    //stageSpawnBehaviours = List<StageSpawnBehaviour>(); (Deprecated)
    stageSpawnBehaviours = [];
    for (var position = Stage.values.indexOf(StageTime.currentStage);
      ...
      StageSpawnBehaviour stageSpawnBehaviour =
          RockCometConfig.getSpawnBehaviour(Stage.values[position]);
  ...
  static StageSpawnBehaviour getSpawnBehaviour(Stage stage) {
    ...
    StageSpawnBehaviour stageSpawnBehaviour = stageSpawnBehaviours
        .firstWhere((element) => element.stageInfo.stage == stage);
