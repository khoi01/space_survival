...
class ReloadWeapon {
  ...
  bool isTriggerReloadSound = false;
  Vehicle vehicle;
  ...
  ReloadWeapon(
    ...
    reloadRect = Rect.fromLTWH(
        setXPosition, setYPosition, widthComponent, heightComponent);
  ...
  void render(Canvas canvas) {
    if (reloadPercentage <= 100 && reloadPercentage > 75) {
      ...
      if (isTriggerReloadSound) {
        Flame.audio.play('sfx/spaceship/sound_reload.mp3', volume: .45);
        isTriggerReloadSound = false;
      }
    }
  }

  void update(double t) {
    reloadPercentage = ((vehicle.nextUsedReload - Time.getCurrentTime()) /
            vehicle.vehicleAttribute.maxUsedInterval *
            100)
        .toInt();
  }
}
